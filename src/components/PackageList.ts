...
export function PackageList(props: PackageListProps) {
	...
	const startIndex = getListWindowStart(
		packages.length,
		visibleItems,
		selectedIndex,
	)
	const visiblePackages = packages.slice(startIndex, startIndex + visibleItems)
	const hasDescription = packages.some(
		(pkg) => pkg.description.trim().length > 0,
	)
	...
	return Box(
		...
		...visiblePackages.map((pkg, offset) => {
			...
			return Box(
				...
				Text({
					content: truncate(pkg.name, columns.nameWidth).padEnd(
						columns.nameWidth,
					),
				...
				Text({
					content: truncate(pkg.version, columns.versionWidth).padEnd(
						columns.versionWidth,
					),
					width: columns.versionWidth,
					fg: isSelected ? theme.selectionForeground : theme.textSecondary,
				}),
				Text({
					content: truncate(pkg.repository, columns.repoWidth).padEnd(
						columns.repoWidth,
					),
				...
				...(columns.showRate
					? [
							Text({
								content: truncate(formatAurRate(pkg), columns.rateWidth).padEnd(
									columns.rateWidth,
								),
