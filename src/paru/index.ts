...
import type { ParuSearchOptions, ParuListOptions } from './types';
import { parseSearchOutput, parseInfoOutput, parseListOutput, parseUpdatesOutput } from './parser';
import { resolvePreAuthCommand, resolvePrivilegedCommand, type AuthMode } from './auth';
...
function runParuCommand(args: string[], options: RunParuCommandOptions = {}): Promise<ParuCommandResult> {
  ...
  const preAuth = resolvePreAuthCommand(resolved.command, resolved.args, paruBinary);

  if (preAuth) {
    try {
      const authResult = spawnSync(preAuth.command, preAuth.args, { stdio: preAuth.stdio });
...
export async function searchPackages(options: ParuSearchOptions): Promise<Package[]> {
...
export async function getInstalledPackages(options: ParuListOptions = {}): Promise<Package[]> {
  const args = ['-Q'];
...
export async function getPackageInfo(packageName: string): Promise<Partial<Package>> {
...
export async function installPackage(packageName: string): Promise<ParuCommandResult> {
  return runParuCommand(['-S', packageName, '--noconfirm'], { requireRoot: true });
}

export async function removePackage(packageName: string, purge: boolean = false): Promise<ParuCommandResult> {
...
export async function updatePackage(packageName: string): Promise<ParuCommandResult> {
  return runParuCommand(['-S', packageName, '--noconfirm'], { requireRoot: true });
}
