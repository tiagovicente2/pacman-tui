...
import { shouldHandleGlobalShortcut, type SearchMode } from './utils/keyboard';
...
async function loadInstalled() {
  ...
  try {
    ...
    setState({ isLoading: false, errorMessage: e.message, statusMessage: 'Error' });
  }
}

async function doSearch(q: string, fromSubmit: boolean = false): Promise<number> {
  ...
  try {
    ...
    const pkgs = sortSearchPackages(await searchPackages({ query: trimmedQuery }), trimmedQuery);
    if (requestId !== searchLoadId || state.currentTab !== 'search') {
      return 0;
    }
    selectedIndex = 0;
    setState({ packages: pkgs, isLoading: false, statusMessage: `${pkgs.length} found` });
    updateSelected();
    if (fromSubmit) {
      if (pkgs.length > 0) {
        enterSearchListMode();
    ...
    setState({ isLoading: false, errorMessage: e.message, statusMessage: 'Search error' });
    if (fromSubmit) {
      enterSearchInputMode();
    }
    return 0;
...
function handleInstall() {
  ...
  showConfirmDialog('Install Package', `Install ${pkg.name} (${pkg.version})?`, async () => {
    hideConfirmDialog();
    setState({ isLoading: true, statusMessage: `Installing ${pkg.name}...`, errorMessage: null });
    try {
...
function handleRemove() {
  ...
  showConfirmDialog('Remove Package', `Remove ${pkg.name} (${pkg.version})?`, async () => {
    hideConfirmDialog();
    setState({ isLoading: true, statusMessage: `Removing ${pkg.name}...`, errorMessage: null });
    try {
      ...
      setState({
        errorMessage: error instanceof Error ? error.message : 'Removal failed',
...
function handleUpdate() {
  ...
  showConfirmDialog(title, message, async () => {
    ...
    setState({
      isLoading: true,
      statusMessage: updateSelectedOnly ? `Updating ${selected!.name}...` : 'Updating system...',
      errorMessage: null,
    });

    try {
      const result = updateSelectedOnly
        ? await updatePackage(selected!.name)
        : await updateSystem();

      if (!result.success) {
        throw new Error(result.error || (updateSelectedOnly ? 'Package update failed' : 'System update failed'));
      }

      setState({
        statusMessage: updateSelectedOnly ? `Updated ${selected!.name}` : 'System updated',
      });
      ...
      setState({
        errorMessage: error instanceof Error ? error.message : (updateSelectedOnly ? 'Package update failed' : 'System update failed'),
...
function render() {
  ...
  if (state.currentTab === 'search') {
    ...
    const pkgList = PackageList({
      ...
      width: listW,
      height: listH,
      onSelect: (i: number) => { selectedIndex = i; updateSelected(); },
      showAurRate: true,
    });
    ...
    content = Box(
      { width, height: contentH, flexDirection: 'column', backgroundColor: theme.surface },
      searchInput,
      Box(
        { width, height: listH, flexDirection: 'row', backgroundColor: theme.surface },
        pkgList,
    ...
    content = Box(
      { width, height: contentH, flexDirection: 'row', backgroundColor: theme.surface },
      pkgList,
  ...
  const topRow = inListTab && (listFilterMode || listFilterQuery.length > 0)
    ? SearchInput({
        query: listFilterQuery,
        width,
        mode: listFilterMode ? 'input' : 'list',
        placeholder: state.currentTab === 'updates'
          ? 'Filter updates...' : 'Filter installed packages...',
      })
    : Box({
        width,
        height: 1,
...
function setupKeys() {
  keypressHandler = (key: any) => {
    if (showConfirm) {
      if (key.name === 'y' || key.name === 'return') { if (confirmAction) confirmAction(); }
      else if (key.name === 'n' || key.name === 'escape') { hideConfirmDialog(); }
      return;
    }

    const inSearchTab = state.currentTab === 'search';
    const inListTab = isListTab(state.currentTab);
    const isPrintable = typeof key.sequence === 'string' && key.sequence.length === 1 && !key.ctrl && !key.meta;
    ...
    if (inSearchTab && searchMode === 'input') {
      ...
      if (key.name === 'backspace') {
        ...
        state = { ...state, searchQuery: query, statusMessage: 'Press Enter to search' };
        render();
        return;
      }
    ...
    if (inListTab && listFilterMode) {
      ...
      const allowTabSwitchWhileFiltering = key.name === 'tab'
        || key.name === '1'
        || key.name === '2'
        || key.name === '3'
        || key.name === 'q'
    ...
    if (key.name === '1') { switchTab('installed'); return; }
    if (key.name === '2') { switchTab('search'); return; }
    if (key.name === '3') { switchTab('updates'); return; }
    
    if (key.name === 'up' || key.name === 'k') {
      if (state.packages.length) { selectedIndex = Math.max(0, selectedIndex - 1); updateSelected(); }
      return;
    }
    if (key.name === 'down' || key.name === 'j') {
      if (state.packages.length) { selectedIndex = Math.min(state.packages.length - 1, selectedIndex + 1); updateSelected(); }
      return;
    }
    if (key.name === 'pageup') {
      if (state.packages.length) { selectedIndex = Math.max(0, selectedIndex - 10); updateSelected(); }
      return;
    }
    if (key.name === 'pagedown') {
      if (state.packages.length) { selectedIndex = Math.min(state.packages.length - 1, selectedIndex + 10); updateSelected(); }
      return;
    }
